...
- Creating prompts for sub-agents or LLM interactions
...
- **Progressive token accumulation**: As the conversation advances through turns, each user message and assistant response accumulates within the context window.
- **Linear growth pattern**: The context usage grows linearly with each turn, with previous turns preserved completely.
- **200K token capacity**: The total available context window represents the maximum capacity for storing conversation history.
- **Input-output flow**: Each turn consists of input phase (history + current message) and output phase (generated response).

### Concise is Key

The context window is a public good. Your prompt, command, skill shares the context window with everything else Claude needs to know.
...
|--------------|----------|---------|
| **High** | Multiple approaches valid | "Analyze the code structure and suggest improvements" |
...
LLMs respond to the same persuasion principles as humans. Understanding this psychology helps you design more effective skills.
...
Obligation to return benefits received. **Use sparingly** - can feel manipulative.

### 7. Liking

Preference for cooperating with those we like. **DON'T USE** for compliance - creates sycophancy.
...
- [ ] **Is it concise?** (Does each token justify its cost?)
- [ ] **Is the freedom level appropriate?** (High for flexible tasks, low for fragile operations)
